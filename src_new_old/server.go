...
// les clients avec leur nom
// l'adrese du endpoint du client "courant"
// son adresse ip
type Client struct {
	name string
	addr net.Addr
...
// type Room avec un unique id
// et le mapping des clients
type Room struct {
	id uuid.UUID
...
// On verifie que deux client
// ne choisisse pas le meme nickname
...
// On genere les rooms
// En leur affectant un unique id
...
func room_operation(conn net.Conn, rooms *map[uuid.UUID]*Room, client Client) error {
	...
	for {
		...
		switch true {
			...
			default:
				fmt.Printf("Unknown command: %s", res)
				message.SendText(conn, options)
		}
	}
